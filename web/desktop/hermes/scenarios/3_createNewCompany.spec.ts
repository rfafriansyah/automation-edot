...
test.describe("Create New Company on Hermes - Positive", () => {
  ...
  test("Create new Company, Country: Malaysia", async ({
    page,
    registerCompanyPage,
  }) => {
    await registerCompanyPage.filltxtCompanyName(
      payloadCreateMalaysia().companyNameMalay
    );
    ...
    await registerCompanyPage.selectdrpdwnLanguage();
    await registerCompanyPage.filltxtStreetAddress(
      payloadCreateMalaysia().addressMalay
    );

    await registerCompanyPage.selectdrpdwnCountry(
      payloadCreateMalaysia().countryMalay
    );
    await registerCompanyPage.fillCountryMalaysia(
      payloadCreateMalaysia().dataMalaysia
    );
    await registerCompanyPage.clickbuttonNext();
    await registerCompanyPage.clickbuttonNext();
  ...
  test("Create new Company, Country: Philippine", async ({
    page,
    registerCompanyPage,
  }) => {
    await registerCompanyPage.filltxtCompanyName(
      payloadCreatePhilippine().companyNamePhilippine
    );
    await registerCompanyPage.filltxtEmail(
      payloadCreatePhilippine().emailPhilippine
    );
    await registerCompanyPage.filltxtPhone(
      payloadCreatePhilippine().phonePhilippine
    );
    await registerCompanyPage.selectdrpdwnIndustryType();
    await registerCompanyPage.selectdrpdwnCompanyType();
    await registerCompanyPage.selectdrpdwnLanguage();
    await registerCompanyPage.filltxtStreetAddress(
      payloadCreatePhilippine().addressPhilippine
    );

    await registerCompanyPage.selectdrpdwnCountry(
      payloadCreatePhilippine().countryPhilippine
    );
    await registerCompanyPage.fillCountryPhilippine(
      payloadCreatePhilippine().dataPhilippine
    );
    await registerCompanyPage.clickbuttonNext();
    await registerCompanyPage.clickbuttonNext();
    await registerCompanyPage.filltxtBranchName(
      payloadCreatePhilippine().branch
    );
    await registerCompanyPage.clickbuttonFillTheSameDataRecords();
  });
