...
export class BadStockSubmenuPage {
  readonly page: Page;
  readonly headerComponent: HeaderComponent;

  readonly titleBadStock: Locator;
  ...
  constructor(page: Page) {
    ...
    this.titleBadStock = page.locator("main>header>div:nth-child(1)>h2");
  ...
  async screenshotListPage() {
    ...
    await expect(this.page).toHaveScreenshot("listBadStock.png", {
    ...
    const actualText = await this.titleBadStock.textContent();
    await expect(actualText).toMatchSnapshot("titleBadStock.txt");
  }
}
