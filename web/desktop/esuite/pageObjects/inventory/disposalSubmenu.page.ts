...
export class DisposalSubmenuPage {
  readonly page: Page;
  readonly headerComponent: HeaderComponent;

  readonly titleDisposal: Locator;
  ...
  constructor(page: Page) {
    ...
    this.titleDisposal = page.locator("main>header>div:nth-child(1)>h2");
  ...
  async screenshotListPage() {
    ...
    await expect(this.page).toHaveScreenshot("listDisposal.png", {
    ...
    const actualText = await this.titleDisposal.textContent();
    await expect(actualText).toMatchSnapshot("titleDisposal.txt");
  }
}
